...
export async function POST(request: Request) {
  try {
    ...
    const { nom, prenom, email, phone, accepteConditions, locale } = body;
    ...
    if (!accepteConditions) {
      return NextResponse.json(
        {
          message: isEn
            ? "You must agree to provide your personal information"
    ...
    const subscriberEmailHtml = isEn
      ? `
        <div style="font-family: 'Open Sans', sans-serif; color: #021373; max-width: 600px; margin: 0 auto; text-align: center;">
          <a href="https://${siteDomain}/en"> <img src="${logoUrl}" alt="Next Impact logo" style="max-width: 80px; margin-bottom: 20px;" /></a>
          <div style="text-align: left;">
            <h2 style="color: #021373; border-bottom: 2px solid #1F54BF; padding-bottom: 10px;">
              Here is your Headless WordPress white paper!
            </h2>
            <p>Hi ${prenom},</p>
            <p>Thank you for your interest in our Headless WordPress white paper. You can find it via the link below.</p>
            <p><a href="https://${siteDomain}/livre_blanc_wordpress_headless.pdf" style="color: #1F54BF; text-decoration: none; font-size:14px">Download the Headless WordPress white paper</a></p>
            <p>We hope this white paper will help you understand the benefits and best practices of Headless WordPress.</p>
            <p>If you have any questions or want to discuss your project, feel free to reply to this email or book a slot.</p>
            <div style="margin: 20px 0; text-align: center;"></div>
            <p>Best regards,<br/>Agathe at Next Impact</p>
            <div style="margin: 20px 0; text-align: center;"></div>
            <a href="https://calendar.app.google/7ZJwqU9AGgk8bCf88" style="background: #F2E57E; color: #021373; padding: 10px 20px; border-radius: 25px; text-decoration: none;">Book an appointment</a>
    ...
    try {
      ...
      await Promise.all([
        ...
        sendMail({
          to: email,
          subject: isEn
            ? "Your Headless WordPress white paper - Next Impact"
            : "Votre livre blanc WordPress Headless - Next Impact",
      ...
      return NextResponse.json({
        message: isEn
          ? "Registration successful! You'll receive your white paper shortly."
      ...
      return NextResponse.json(
        {
          message: isEn
            ? "Error sending email. Please check the SMTP configuration."
