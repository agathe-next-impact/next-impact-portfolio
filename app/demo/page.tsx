...
const demoVideos = [
  ...
  },
  {
    ...
    projectDescription: "Une plateforme pour les États Généraux Communaux utilisant WordPress Headless avec Next.js",
  },
];
            ...
            <div className="mt-36 grid grid-cols-1 gap-8">
              {demoVideos.map((video, idx) => (
                  ...
                  <div className="col-span-1 md:col-span-2 flex flex-col justify-center items-start p-6 bg-mediumblue/40 backdrop-blur-md h-full">
                    <div className="mb-4">
                      <h4 className="text-2xl md:text-xl font-semibold text-white mb-2">{video.projectName}</h4>
                      <p className="text-base text-white/80 mb-2">{video.projectDescription}</p>
                    </div>
                    {video.projectLink && (
                      <Link href={video.projectLink} className="text-coral font-medium underline hover:text-coral/80 transition">
                        Voir le projet
                      </Link>
                    )}
                  </div>
