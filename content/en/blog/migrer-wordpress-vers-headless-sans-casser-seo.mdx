...
On the `robots.txt` side, explicitly allow AI search crawlers (OAI-SearchBot, PerplexityBot, ClaudeBot, GPTBot) or block them — but do it consciously. Mind the nuance: GPTBot is used for model training, while OAI-SearchBot is the one powering ChatGPT Search — blocking one does not have the same effect as blocking the other.
