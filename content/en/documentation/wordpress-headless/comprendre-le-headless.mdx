...
<TldrCallout label="TL;DR">
  A headless architecture separates content management (the backend) from content display (the frontend). In headless WordPress, WordPress keeps its role as a CMS and exposes content through an API (REST or GraphQL), while a separate frontend — for example Next.js — renders the public pages. That separation unlocks performance and design freedom that a PHP theme cannot match.
</TldrCallout>
