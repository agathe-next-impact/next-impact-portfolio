...
<TldrCallout label="TL;DR">
  Next.js offers three rendering strategies usable with headless WordPress: SSG (HTML built at build time, ideal for stable pages), SSR (HTML built on every request, for personalized content), and ISR (incremental regeneration, combining the speed of static with automatic refresh). The three can be mixed inside a single site, page by page.
</TldrCallout>
