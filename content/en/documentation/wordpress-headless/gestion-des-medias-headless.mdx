...
<TldrCallout label="TL;DR">
  In headless WordPress, the media library remains the single upload tool on the editor side. Images are exposed through the API (`/wp-json/wp/v2/media` or WPGraphQL) and rendered on the Next.js side with the `next/image` component, which adds WebP/AVIF conversion, on-demand resizing, lazy loading and blur placeholders. The result: LCP under 2.5 s even on visually rich sites.
</TldrCallout>
