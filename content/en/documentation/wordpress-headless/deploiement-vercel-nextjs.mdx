...
<TldrCallout label="TL;DR">
  To ship a headless WordPress site, deploy the Next.js frontend on Vercel (or Netlify, Cloudflare Pages) while WordPress stays on its usual PHP or managed host. Vercel connects to the Git repository, builds on every push and serves the site from a global CDN. The link to the backend is handled through environment variables holding the WordPress API URL.
</TldrCallout>
