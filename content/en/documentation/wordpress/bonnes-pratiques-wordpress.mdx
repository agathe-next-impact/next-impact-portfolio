...
For a WooCommerce e-commerce site, choose a theme explicitly declaring its WooCommerce compatibility: integration of shop, product, cart and checkout templates. Astra Pro and GeneratePress Premium offer these native integrations. Verify that the theme supports schema markup (structured data) for products, reviews and prices: this markup helps Google and AI answer engines understand your catalog precisely — a machine-comprehension matter first and foremost, as enriched display in search results has become marginal and is never guaranteed.
...
First, install an SEO plugin (Yoast SEO or SEOPress). These plugins allow customizing the `<title>` and `<meta description>` tags of each page, automatically generating the XML sitemap (file that lists all the site's URLs for indexing robots), and adding schema.org structured data. Since Google retired the FAQ and then HowTo rich results, the point of this markup is no longer how your listing looks: it helps engines — classic and AI alike (ChatGPT, Perplexity, AI Overviews) — understand and cite your pages correctly.
