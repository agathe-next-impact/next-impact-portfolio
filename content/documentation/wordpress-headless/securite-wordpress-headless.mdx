...
<TldrCallout>
  Sécuriser un WordPress headless repose sur trois mesures : isoler le back-office WordPress sur un sous-domaine non public (par exemple `admin.monsite.fr` protégé par IP ou Basic Auth), exposer uniquement les endpoints API nécessaires (lecture publique, écriture authentifiée par JWT), et placer le front-end Next.js derrière un CDN avec en-têtes de sécurité (CSP, HSTS, X-Frame-Options). La surface d'attaque diminue mécaniquement.
</TldrCallout>
