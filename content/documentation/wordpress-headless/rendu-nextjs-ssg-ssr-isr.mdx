...
<TldrCallout>
  Next.js propose trois stratégies de rendu utilisables pour un WordPress headless : SSG (HTML généré au build, idéal pour les pages stables), SSR (HTML généré à chaque requête, pour le contenu personnalisé), et ISR (régénération incrémentale, qui combine la rapidité du statique avec un rafraîchissement automatique). On peut mixer les trois au sein d'un même site, page par page.
</TldrCallout>
