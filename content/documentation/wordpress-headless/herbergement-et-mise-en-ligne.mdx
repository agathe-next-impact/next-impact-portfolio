...
<TldrCallout>
  Un site WordPress headless s'héberge en deux blocs distincts : le back-end WordPress sur un hébergement PHP/MySQL (mutualisé, managé ou serveur dédié) et le front-end Next.js sur une plateforme JAMstack avec CDN (Vercel, Netlify, Cloudflare Pages). Les deux blocs communiquent par HTTPS et peuvent évoluer indépendamment, ce qui simplifie les montées en charge.
</TldrCallout>
