...
<TldrCallout>
  Pour un site WordPress headless multilingue, le travail se partage entre deux couches : WordPress gère les traductions de contenu via WPML ou Polylang (et les expose à l'API par langue), tandis que Next.js gère le routage localisé (`/fr`, `/en`), les chaînes d'interface (next-intl, next-translate) et les balises `hreflang`. Cette répartition préserve les habitudes éditoriales sans sacrifier le SEO international.
</TldrCallout>
